...
def make_pdf(path, password=None, algorithm='RC4-40'):
    ...
    for i in range(2):
        c.drawString(30, 550, 'Hiraku test page ' + str(i + 1))
...
class CoreTests(unittest.TestCase):
    ...
    def test_office_decryption_roundtrip(self):
        for extension in ('.docx','.xlsx','.pptx'):
            ...
            if extension == '.docx':
                doc=Document();doc.add_paragraph('Hiraku');doc.save(source)
            elif extension == '.xlsx':
                doc=Workbook();doc.active['A1']='Hiraku';doc.active['B1']='=1+2';doc.save(source)
            else:
                doc=Presentation();slide=doc.slides.add_slide(doc.slide_layouts[0]);slide.shapes.title.text='Hiraku';doc.save(source)
