...
const {chromium} = require(require.resolve('playwright', {paths: [process.env.HIRAKU_NODE_MODULES || process.cwd()]}));
...
(async () => {
  ...
  try {
    ...
    browser = await chromium.launch({channel: process.env.HIRAKU_BROWSER || 'msedge', headless: true});
    ...
    if (context && base) await context.request.post(base + '/api/shutdown', {headers: {'X-Hiraku': '1'}, data: {}}).catch(() => {});
