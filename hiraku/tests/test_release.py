...
class ReleaseTests(unittest.TestCase):
    def test_private_and_binary_paths_rejected(self):
        for path in ('../secret', 'outputs/result.txt', 'hiraku/.test-data/file.txt',
                     'hiraku/vendor/john/LICENSE.txt', 'hashcat.exe', 'driver.dll', '.git/config'):
    ...
    def test_allowlist_determinism_and_tampering(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / 'hiraku').mkdir()
            (root / 'hiraku/release-files.json').write_text(json.dumps({'version': 'test', 'files': ['README.md']}))
            ...
            with zipfile.ZipFile(first, 'a') as archive:
                archive.writestr('Hiraku-test/secret.txt', 'forbidden')
            with self.assertRaises(ValueError):
                verify(first, root)
