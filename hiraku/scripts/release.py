...
TEXT_SUFFIXES = {'.py', '.ps1', '.sh', '.cmd', '.md', '.txt', '.json', '.js', '.cjs', '.html', '.css', '.webmanifest'}
...
def specification(root):
    spec = json.loads((root / 'hiraku/release-files.json').read_text(encoding='utf-8'))
