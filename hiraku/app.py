"""Hiraku: local file recovery and conversion workbench."""
...
DEFAULT_DATA = APP_DIR.parent / 'outputs' / 'hiraku'
    ...
    def discover_hashcat(self):
        ...
        if config.exists():
            try:
                ...
                pass
        candidates = [os.environ.get('HIRAKU_HASHCAT', ''),
...
class Handler(BaseHTTPRequestHandler):
    ...
    def authorized(self):
        ...
        try:
            ...
            return False
        value = cookie.get('hiraku_session')
    ...
    def get_route(self):
        ...
        if url.path == '/launch':
            ...
            if self.headers.get('Host') == self.server.authority and secrets.compare_digest(supplied, self.server.token):
                ...
                self.send_data(303, b'', extra={'Location': '/', 'Set-Cookie': f'hiraku_session={self.server.token}; HttpOnly; SameSite=Strict; Path=/' + secure})
                return
        if not self.authorized():
            self.send_data(403, {'error': 'Hiraku.cmdからアプリを起動してください。'})
            return
        path = url.path
        if path == '/api/jobs':
            ...
            self.send_data(200, {'hashcat': str(self.library.hashcat or ''), 'output_dir': str(self.library.root),
                                 ...
                                 'version': '0.1.0-alpha.1', 'connection': self.server.origin, 'tls': self.server.tls})
        elif path == '/api/licenses':
            texts = ['Hiraku third-party notices\n' + (APP_DIR / 'THIRD_PARTY.md').read_text(encoding='utf-8'),
                     'Hiraku license\n' + (APP_DIR / 'LICENSE').read_text(encoding='utf-8')]
            for file in sorted((APP_DIR / 'licenses').glob('*.txt')):
    ...
    def do_POST(self):
        if not self.authorized() or self.headers.get('X-Hiraku') != '1':
...
def main():
    ...
    if instance is None:
        if not args.no_browser:
            try:
                ...
                pass
        print('Hiraku is already running for this data directory.', flush=True)
        return
    ...
    print('Hiraku is running at ' + server.origin, flush=True)
    if not args.no_browser:
        webbrowser.open(url)
    try:
