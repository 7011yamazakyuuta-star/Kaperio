...
def validate_plan(data):
    ...
    plan = {'strategy': strategy, 'minutes': minutes, 'workload': workload,
            'temperature': temperature, 'devices': devices}
...
def run_hashcat(executable, folder, mode, plan, stop, update, resume=False):
    ...
    base = [str(executable), '--session', 'hiraku_' + folder.name, '--restore-file-path', str(restore)]
    if resume and restore.exists():
        args = base + ['--restore']
    else:
        attack = write_inputs(folder, plan)
        args = base + ['-m', str(mode), str(folder / 'source.hash')] + attack
        args += ['--status', '--status-json', '--status-timer', '1', '--potfile-disable',
                 '--logfile-disable', '--outfile', str(found), '--outfile-format', '3',
                 '-w', str(plan['workload']), '--hwmon-temp-abort', str(plan['temperature'])]
        if plan['devices']:
            args += ['-d', plan['devices']]
        if plan['strategy'] == 'mask' and mode in (10400, 10500, 10600, 10700):
            args += ['-O']
