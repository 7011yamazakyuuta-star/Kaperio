...
if (-not (Test-Path -LiteralPath $pythonExe)) {
    throw 'Python environment missing. Run Setup.cmd first. See hiraku/README.md.'
}
$statePath = Join-Path (Split-Path $appRoot) 'outputs\hiraku\launch.json'
if (Test-Path -LiteralPath $statePath) {
    ...
    if (Get-Process -Id $launch.pid -ErrorAction SilentlyContinue) {
        try {
            $response = Invoke-WebRequest -Uri $launch.url -UseBasicParsing -SessionVariable hirakuSession -TimeoutSec 5
