...
アプリ本体はMITライセンスです。著作権者表示はKaperio contributorsとしています。
...
ソースZIPではPython依存パッケージはセットアップ時に利用者が取得します。
デスクトップ配布物ではPythonと依存ライブラリを同梱し、実際のwheelに含まれる許諾表示、
PDFiumなどのネイティブ部品の表示、Python/OpenSSLの条件とバージョン一覧を保持します。
ビルド・署名・対応OSの詳細は [DESKTOP.md](DESKTOP.md) を参照してください。
...
python kaperio/scripts/release.py verify outputs/releases/Loxmit-0.4.0-alpha.2-source.zip
```
