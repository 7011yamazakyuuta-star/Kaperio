...
def main():
    ...
    try:
        ...
        for ext in ('.docx','.xlsx','.pptx'):
            ...
            if ext=='.docx':
                doc=Document();doc.add_paragraph('Kaperio acceptance test');doc.save(plain)
            elif ext=='.xlsx':
                doc=Workbook();doc.active['A1']='Kaperio';doc.active['B1']='=2+3';doc.save(plain)
            else:
                doc=Presentation();slide=doc.slides.add_slide(doc.slide_layouts[0]);slide.shapes.title.text='Kaperio';doc.save(plain)
            encrypted=ROOT/('encrypted'+ext)
            with plain.open('rb') as src,encrypted.open('wb') as dest:
                msoffcrypto.OfficeFile(src).encrypt('Kaperio42',dest)
        ...
        with pyzipper.AESZipFile(str(encrypted),'w',encryption=pyzipper.WZ_AES,compression=pyzipper.ZIP_DEFLATED) as z:
            z.setpassword(b'Kaperio42');z.writestr('folder/acceptance.txt',b'Original content\n'*10)
        sources.append(encrypted)
        for algorithm in ('RC4-40', 'RC4-128', 'AES-128', 'AES-256-R5', 'AES-256'):
            encrypted=ROOT/(algorithm+'.pdf');make_pdf(encrypted,'Kaperio42',algorithm);sources.append(encrypted)
