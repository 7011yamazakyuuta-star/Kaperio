...
public static class KaperioWindow {
...
function Record-Instance($App, [bool]$Empty, [string]$ProcessName) {
    ...
    if ($null -ne $window -and $window -ne 0) {
        [void][KaperioWindow]::GetWindowThreadProcessId([IntPtr]$window, [ref]$officePid)
    } else {
...
function Trace-Stage([string]$Stage) { Write-Output ('KAPERIO_STAGE=' + $Stage) }
try {
    Trace-Stage 'create'
    switch ($extension) {
        {$_ -in '.docx','.docm'} {
